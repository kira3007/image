...
function setPoint(type,offset, max){
    ...
    ret = ret.map(function(item){
        return getBound(item, _this.width, _this.height);
    });
...
function getBound(arr, width, height){
    ...
    //裁剪，有1px的偏差？
    //多算1px
    xMax.x = Math.min(width, xMax.x + 1);
    yMax.y = Math.min(height, yMax.y + 1);
    ...
    for(var i = 0; i < rowRectange.length; i++){
        for(var j = 0; j < colRectange.length; j++) {
            ...
            //if(a.checkOverlap(b)){
                ...
                if((a.lowerBound.x == b.lowerBound.x) && (a.upperBound.x <= b.upperBound.x)){
